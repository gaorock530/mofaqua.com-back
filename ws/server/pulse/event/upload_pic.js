...
module.exports = async (socket, data, pulse) => {
  ...
  if (data.i !== -1 && socket.writeFile[data.n].index === data.i) {
    ...
    try {
      if (data.c === 'tn') {
        out = `https://localhost:${process.env.PORT}/images/thumbnails/${socket.UID}/${socket.writeFile[data.n].name}.thumbnail.jpeg`;
        ...
        out = `https://localhost:${process.env.PORT}/images/channel-cover/${socket.UID}/${socket.writeFile[data.n].name}.thumbnail.jpeg`;
